...
''}
  ...
  firmwareListFile = pkgs.writeText "firmware-list.txt" (
      lib.concatLines "\n"
        (fwFile:
          let relative = head (match ".*/lib/firmware/(.*)" (toString fwFile)); in
          "${fwFile}:${relative}"
        )
        cfg.firmwareFiles
    );
  ...
  initrd = pkgs.stdenv.mkDerivation {
    ...
    buildPhase = ''
      ...
      ${lib.optionalString (cfg.firmwareFiles != []) ''
        ...
        ${lib.concatMapStringsSep "\n" (fwFile: 
          let 
            relativePath = head (match ".*/lib/firmware/(.*)" (toString fwFile));
          in ''
