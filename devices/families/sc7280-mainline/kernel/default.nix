...
let
  ...
  configfile = stdenv.mkDerivation {
    ...
    buildPhase = ''
      ...
      # Merge sc7280.config fragment if it exists
      if [ -f arch/arm64/configs/sc7280.config ]; then
        scripts/kconfig/merge_config.sh -m .config arch/arm64/configs/fairphone.config
      fi
