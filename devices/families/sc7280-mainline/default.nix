...

{
  ...
  mobile.boot.stage-1.firmware = [
    (pkgs.runCommand "initrd-firmware" {} ''
      ...
      find $out/lib/firmware/qcom/${config.mobile.device.dtbSocPrefix} -name "modem.mbn" -delete

      rm -rf $out/usr/share/qcom/qcm6490/Fairphone/fp5/sensors/config
      rm -rf $out/lib/firmware/qcom/qcm6490/fairphone5/modem_pr

      # Copy extra a660 firmware from linux-firmware
  ...
  mobile.system.android = {
    ...
    bootimg = {
      header_version = lib.mkDefault "2";
      dtb_offset = lib.mkDefault "0x01f00000";
      flash = {
    ...
    bootimg.dtb = lib.mkDefault "dtbs/qcom/${config.mobile.device.dtbSocPrefix}-${config.mobile.device.name}.dtb";
  };
